import {
  ...
  isGroupedTag,
  parseGroupedTag,
} from "../config/tagGroups";

export const processRawTags = (rawTags = []) => {
...
export const processAllAlgorithmTags = (data = []) => {
  ...
  supportedGroups.forEach((groupId) => {
    ...
    const hasAlgorithmWithoutGroup = Array.from(algorithmGroups.values()).some(
      (groups) => !groups.has(groupId)
    );
...
export const createAlgorithmTagProcessor = () => {
